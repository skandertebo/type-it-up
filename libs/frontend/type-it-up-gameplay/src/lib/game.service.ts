...
export class GameService {
  ...
  getNewGame(gameOptions: GameOptions): Observable<Game> {
    return this.apolloClient
      .query<{ generateText: { data: { text: string } } }>({
        query: GENERATE_TEXT,
        variables: {
      ...
      .pipe(map((result) => ({ text: result.data.generateText.data.text })));
  }
  ...
  calculateMetrics(words: Word[], time: number) {
    ...
    for (const word of words) {
      ...
      for (const char of word) {
        if (char.state === CharState.Deactivated) {
          finished = true;
          break;
        }
    ...
    const accuracy =
      totalWrittenChars === 0 ? 0 : totalRightChars / totalWrittenChars;
    return { wpm, accuracy };
  }
  ...
  calculateGameResults(words: Word[], gameOptions: GameOptions): GameResults {
    ...
    const multiplier =
      difficultyMultiplier[gameOptions.difficulty] *
      (1 +
        (gameOptions.numbers ? optionMultiplier : 0) +
